...
GO
SELECT DV.TENDV, CT.SOLUONG, CT.GIADV, SUM(CT.SOLUONG * CT.GIADV)
FROM CHITIETHD CT
...
DROP TRIGGER trg_TaoHoaDonSauKhiTraPhong;
...
AS
BEGIN
    INSERT INTO CHITIETHD (MAHD, MADV, SOLUONG, GIADV)
    SELECT 
        i.MAHD,
        SD.MADV,
        SD.SOLUONG,
        DV.DONGIA
    FROM inserted i
    JOIN DATPHONG DP ON DP.MADATPH = i.MADATPH
    JOIN SUDUNGDV SD ON SD.MADATPH = DP.MADATPH
    JOIN DICHVU DV ON DV.MADV = SD.MADV
    WHERE SD.TRANGTHAI = N'Đã hoàn thành';
END;
GO
...
DROP FUNCTION fn_TongTienDichVu
...
BEGIN
    ...
    WHERE SD.MADATPH = @MaDatPh AND SD.TRANGTHAI = N'Đã hoàn thành';
...
GRANT SELECT, INSERT ON KHACH TO Role_NhanVien;
GRANT SELECT ON LOAIPHONG TO Role_NhanVien;
GRANT SELECT, UPDATE ON PHONG TO Role_NhanVien;
GRANT SELECT ON DICHVU TO Role_NhanVien;
GRANT SELECT, INSERT, UPDATE ON DATPHONG TO Role_NhanVien;
GRANT SELECT, INSERT ON SUDUNGDV TO Role_NhanVien;
GRANT SELECT ON HOADON TO Role_NhanVien;
GRANT SELECT ON CHITIETHD TO Role_NhanVien;
...
TO DISK = 'D:\Backup\QLKS_FULL.bak'
WITH FORMAT,
     INIT,
     NAME = 'Full Backup QLKS';

BACKUP DATABASE QLKS
TO DISK = 'D:\Backup\QLKS_DIFF.bak'
WITH DIFFERENTIAL,
     INIT,
     NAME = 'Differential Backup QLKS';

BACKUP LOG QLKS
TO DISK = 'D:\Backup\QLKS_LOG.trn'
WITH INIT,
     NAME = 'Log Backup QLKS';

BACKUP LOG QLKS
TO DISK = 'D:\Backup\QLKS_TAILLOG.trn'
WITH NO_TRUNCATE,
     INIT,
     NAME = 'Tail Log Backup QLKS';
...
FROM DISK = 'D:\Backup\QLKS_FULL.bak'
WITH NORECOVERY, REPLACE;


RESTORE DATABASE QLKS
FROM DISK = 'D:\Backup\QLKS_DIFF.bak'
WITH NORECOVERY;

RESTORE LOG QLKS
FROM DISK = 'D:\Backup\QLKS_LOG.trn'
WITH NORECOVERY;

RESTORE LOG QLKS
FROM DISK = 'D:\Backup\QLKS_TAILLOG.trn'
WITH NORECOVERY;

RESTORE DATABASE QLKS WITH RECOVERY;
